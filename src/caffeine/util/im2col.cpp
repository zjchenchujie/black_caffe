...
void im2col_cpu(const Dtype* data_im, const int channels, const int height, const int width, const int ksize,
        const int stride, Dtype* data_col){
    int height_col = (height - ksize) / stride + 1;
    int width_col = (width - ksize) / stride + 1;
    int channel_col = channels * ksize * ksize;
    for(int c = 0; c < channel_col; ++c){
        ...
        for(int h = 0; h < height_col; ++h){
            for(int w = 0; w < width_col; ++w){
                ...
                = data_im[(c_im * height + h * stride + h_offset) * width + w*stride + w_offset];
            }
        }
    }
}
