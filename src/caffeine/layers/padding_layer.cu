...
namespace caffeine{
    ...
    void PaddingLayer<Dtype>::SetUp(const vector<caffeine::Blob<Dtype> *> &bottom,
        ...
        HEIGHT_OUT_ = HEIGHT_IN_ + 2 * PAD_;
        WIDTH_OUT_ = WIDTH_IN_ + 2 * PAD_;
        (*top)[0]->Reshape(NUM_, CHANNEL_, HEIGHT_OUT_, WIDTH_OUT_);

    }
    template <typename Dtype>
    void PaddingLayer<Dtype>::Forward_cpu(const vector<caffeine::Blob<Dtype> *> &bottom,
                                   vector<caffeine::Blob<Dtype> *> *top) {
        ...
        for(int n = 0; n < NUM_; ++n){
            for(int c = 0; c < CHANNEL_; ++c){
                for(int h = 0; h < HEIGHT_IN_; ++h){
                    memcpy(top_data + (((n * CHANNEL_) + c) * HEIGHT_OUT_ + h + PAD_) * WIDTH_OUT_ + PAD_,
                            bottom_data + ((n * CHANNEL_ + c) * HEIGHT_IN_ + h) * WIDTH_IN_, sizeof(Dtype)*WIDTH_IN_);
                }
            }
        }
    }
    ...
    template <typename Dtype>
    Dtype PaddingLayer<Dtype>::Backward_cpu(const vector<caffeine::Blob<Dtype> *> &top, const bool propagate_down,
        ...
        for(int n = 0; n < NUM_; ++n){
            for(int c = 0; c < CHANNEL_; ++c){
                for(int h = 0; h < HEIGHT_IN_; ++h){
                    memcpy(bottom_diff+((n*CHANNEL_+c)*HEIGHT_IN_+h)*WIDTH_IN_,
                            top_diff+((n*CHANNEL_+c)*HEIGHT_OUT_+h+PAD_)*WIDTH_OUT_+PAD_,
