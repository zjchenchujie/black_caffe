...
namespace caffeine {
    ...
    template <typename Dtype>
    void DropoutLayer<Dtype>::Forward_cpu(const vector<caffeine::Blob<Dtype> *> &bottom,
        ...
        if(Caffeine::phase() == Caffeine::TRAIN){
            ...
            viRngBernoulli(VSL_RNG_METHOD_BERNOULLI_ICDF, Caffeine::vsl_stream(), count, mask, 1. - threshold_);
    ...
    template <typename Dtype>
    Dtype DropoutLayer<Dtype>::Backward_cpu(const vector<caffeine::Blob<Dtype> *> &top, const bool propagate_down,
